...
\end{aligned}
\right. \]
\quad Il est alors facile de déduire des bases pour les espaces de $n$, $p$ et $\begin{pmatrix} m & 0 \\ 0 & q \end{pmatrix}$ qui sont respectivement: 
\begin{center}
$\left \{
   \begin{array}{r c l}
...
Soit $I \ne$ $\{0 \}$ un idéal de $L$ et $ax+by+ch$ un élément non nul de $I$. En appliquant  $ad$ $x$ deux fois, on obtient $-2bx\in I$, de même en appliquant $ad$ $y$ deux fois, on obtient $-2ay\in I$. Donc si $a \ne 0$ et $b \ne 0$, alors $x \in I $ ou $y \in I$. Ainsi $I=L$.

Sinon, si $a = b = 0$ alors $ch \in I$, donc $h \in I$. On conclu $I=L$, donc $L$ est simple.
\end{exe}

Dans la section (\ref{d}) nous verrons des algèbres de Lie qui ne sont pas simples, notamment $\ttt(n,\KK)$ et $\nn(n,\KK)$.\\
\\
...
\end{center}
Remarquons que $L$ abélienne équivaut à $L^{(1)} = \{0\}$
\begin{df}
...
\begin{rmq}
\begin{itemize}
\item[•]  $L$ abélien $\Rightarrow L$ résoluble car $[L,L] = 0$.
...
Au début de ce chapitre nous avons instiqué sur l'importance que $car$ $\KK = 0$, nous allons maintenant donner un exemple de ce phenoméne :

\begin{exe}
\quad Dans cet exemple le corps sera $\FF = \ZZ /3 \ZZ$. On a alors $car$ $\FF = 3$. Considérons les matrice de $\gl(3,\FF)$ suivantes: 
\begin{center}
$x = \begin{pmatrix}
0 & 1 & 0 \\
0 & 0 & 1 \\
...
\end{pmatrix} = x $.\\
\\
\quad Montrons que l'espace $L$ engendré par $x$ et $y$ est une sous-algèbre de Lie résoluble. Soient $z_1,z_2 \in L$, tels que $z_1 = a_1x+b_1y$ et $z_2=a_2x+b_2y$ avec $a_i,b_i \in \FF$. On a:
\begin{align*}
...
\end{align*}
\quad $L$ est bien une sous algèbre de Lie. De plus comme $[x,y]=x$, on a que $[L,L]$ est engendré par $x$. Ainsi $[L,L]$ est abélienne, donc $L$ résoluble. 
...
\quad Il est alors évidant qu'aucun de ces trois vecteurs est vecteur propre de $x$. 
...
\quad Nous pouvons obtenir un critère puissant en se basant sur la trace de certains endomorphismes de $L$. Il est évidant que $L$ va être résoluble si $[LL]$ est nilpotente (c'est un conséquence du corollaire (\ref{cor:L3})).
