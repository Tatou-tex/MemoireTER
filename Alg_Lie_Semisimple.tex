...
[x,[y,z]] & = -[y,[z,x]]-[z,[x,y]]
\end{align*}
or comme $L^{(i)}$ est un idéal, on a $[z,x],[x,y] \in L^{(i)} \Rightarrow [y,[z,x]],[z,[x,y]] \in L^{(i)}$, donc $[x,[y,z]] \in L^{(i+1)}$. 
...
Si $x \in End$ $V$, on dit que $x$ \textbf{stablilise} (ou laisse invariant) le drapeau précédent si $x.V_{i} \subset V_{i}$, $\forall i$.
\end{df}
