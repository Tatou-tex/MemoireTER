...
\newtheorem*{cc}{Conclusion}
\newtheorem*{pblm}{Probl\'ematique}
%\newshadetheorem{pblm}{Probl\'ematique}
%---PAGE-DE-GARDE--------------------------------------------------------------

\begin{Large}
\title{\textbf{\bf \scalebox{1.5} {Algèbre de Lie Semisimple}}}
...
\begin{rmq}
\quad (L1) et (L2) appliqué à $[x+y,x+y]$ implique l’anticommutative, i.e. $[x,y]=-[y,x]$. De plus, si $car(\KK) \ne 2$ on a équivalence entre (L2) et $[x,y]=-[y,x]$, $\forall x,y \in L$.
\end{rmq}

\begin{ex}
\begin{enumerate}
\item Tout espace vectoriel muni du crochet nul, i.e . $[x,y]=0$, $\forall x,y \in L $, est une algèbre de Lie. Elle est appelée algèbre $\textbf{abélienne} $ ou $\textbf{commutatif}$.
...
\\
$ A_{\ell} $ : dans ce cas \textbf{$dim$ $V= \ell +1$}, nous noterons $\ssl(V)$ ou $\ssl(\ell+1,\KK)$ l'ensemble des endomorphismes de $V$ dont la trace est nulle.

Rappelons que la trace d'une matrice est la somme de ses éléments diagonaux et qu'elle est indépendante du choix de la base. Comme nous pouvons identifier les matrice au endomorphisme, la trace a du sens pour les endomorphismes de $V$.
...
D'où $\ssl(V)$ est bien une sous-algèbre de Lie, appelée l'\emph{\textbf{algèbre linéaire spéciale}}. % (en rapport avec le groupe linéaire spécial).
Calculons la dimension de $\ssl(V)$:
\begin{center}
$\left \{
   \begin{array}{r c l}
e_{ij }\quad 1\leq i \neq j \leq \ell \\
e_{ii}-e_{i+1;i+1} \quad 1\leq i \leq \ell-1 .
   \end{array}
   \right .$
est une base de $\ssl(\ell+1,\KK)$.
\end{center}
Donc $dim$ $\ssl(V) = \ell^2-1$.
...
\quad Avant de commencer l'exemple $ C_{\ell} $, rappelons les définitions suivantes :
...
\begin{rmq} 
\quad Dans  les espaces vectoriels de dimension paire, si $B$ est une forme bilinéaire antisymétrique et non dégénérée, alors il existe une base dans laquelle, $[ B ]_\mathcal{B}=\begin{pmatrix} 0 & I_\ell \\ -I_\ell & 0 \end{pmatrix} $.
\end{rmq}

$ C_{\ell} $ : dans ce cas $dim$ $V=2 \ell$. Définisons l'\emph{\textbf{algébre sympléctique}}, notée $\ssp (2\ell,\KK)$, comme l'ensemble des endomorphismes $x \in \gl(V)$ qui satisfont $f(x(v),w)=-f(v,x(w))$ pour $f$ une forme bilinéaire antisymétrique non-dégénérée.
...
f([x,v]v ,w) &= f(xy(v),w)-f(yx(v),w) \\
           ...
           &=-f(v,[x,y]x)
\end{align*} 
%Si on fixe une base $\mathcal{B}=(v_1,....,v_{2\ell})$ de $V$. % La matrice de la forme $B$ dans cette base est donnée par $[ B ]_\mathcal{B}=(B(v_i,v_j),\quad 1\leq i , j \leq 2\ell $.
En terme matriciel:
...
\end{aligned}
\right. \]
\quad Il est alors facile de déduire des bases pour les espaces de $n$, $p$ et $q$, qui sont respectivement : 
$\left \{
   \begin{array}{r c l}
...
\\
$ B_{\ell} $: dans ce cas $dim$ $V=2\ell+1$ et $car(\KK) \ne 2$. On nomme \emph{\textbf{algèbre orthogonale}} impair, notée  $\oo(2\ell+1,\KK)$, l'ensemble des $x \in \gl(V)$ tels que $f(x(v),w)=-f(v,x(w))$ pour $f$ une forme bilinéaire symétrique non-dégénérée.

On reprend la même idée que pour $ C_{l} $ :
...
\\
$ D_{l} $ : dans ce cas $dim$ $V=2 \ell$ et $car(\KK) \ne 2$. On nomme l'\textbf{\emph{algèbre orthogonale}} pair, notée  $\oo (2\ell,\KK)$, l'ensemble $ \left \{ x \in \gl(2\ell+1, \KK) \mid sx= -x^Ts, \quad s =\begin{pmatrix} 0 & I_\ell \\ I_\ell & 0 \end{pmatrix} \right \}$.

Les calcules de bases et de dimensions se font de manière similaire à $B_{\ell}$. Nous donnerons donc le dimension de $\oo(2\ell,\KK)$ sans plus de précision, $dim$ $\oo(2\ell,\KK)=2\ell^2-\ell$.\\
\\
...
\item $A=L$ une algèbre de Lie avec $x. y= [x y]$.
...
%\item $L=vect (\{x\})$ avec $\mathcal{B}_L=\{x\}$ \quad $ [ ad$ $x ]_\mathcal{B}  = [ ad$ $x(x) ]_\mathcal{B} = [0]$
%\item $L=\ssl(2,\CC)$ avec $\mathcal{B}= \left \{ e=\begin{pmatrix} 0 & 1\\ 0 & 0 \end{pmatrix}\quad h=\begin{pmatrix} -1 & 0\\ 0 & -1 \end{pmatrix}\quad f=\begin{pmatrix} 0 & 0\\ 1 & 0 \end{pmatrix} \right \}$ \\
%$[ad$ $e]_\mathcal{B} = [ ad$ $e(e)\quad  ad$ $e(h) \quad ad$ $e(f) ] = \begin{pmatrix} 0 & -2 & 0 \\ 0 & 0 &1 \\ 0 & 0 & 0 \end{pmatrix} $
%\end{enumerate}
\end{ex}

\section{Idéal et homomorphismes}
...
\begin{df}
\quad Soit $L$ une algèbre de Lie. On appel l'\textbf{algèbre dérivée} de $L$ est le sous espace $[L,L]$
\end{df}
...
\item $Z(L) = \{ z \in L  \mid [x,z]=0 \quad \forall x \in L\}$ est appelé le \textbf{centre} de $L$ est un idéal de $L$.
...
\item[•]$[x,[i,j]]$=-$[i,[j,x]]$-$[j,[x,i]]$ = -$[j,j']$-$[j,i']$ $\in [I,J]$ ( Jacobi)
\end{itemize}
...
La construction de l'algèbre $L/I$ est formellement le même que le quotion d'anneaux :
...
\subsection{Homomorphisme et représentation}
...
%\begin{df}
%Un \textbf{automorphisme} de $L$ est un isomorphisme de $L$ dans lui-même.
%\quad On note $Aut$ $L$ le groupe des automorphismes de $L$.
%\end{df}
%\begin{ex}
%Soit L une sous-algèbre de $\gl(V)$, si $g \in Gl(V)$ et si $gLg^{-1}=L$, alors $x \longmapsto gxg^{-1}$ est automorphisme de L.
%\end{ex}
%Maintenant nous nous intéressons au cas où $F$ est de caractéristique 0.
%\begin{df}
%on dit que $x\in L$ est nilpotent si $\exists k \in \NN^* $ tel que $ x^k=0$ 
%\end{df}
%Soit $x\in L$ tel que $adx$ est nilpotent, la série exponentielle pour la transformation linéaire sur $\CC$ a du sens sur$F$ puiqsu'il y a un nombre finies de termes:\quad \\$\exp (\text { ad } x)=1+$ ad $x+(\text { ad } x)^{2} / 2 !+(\text { ad } x)^{3} / 3 !+\ldots+(\text { ad } x)^{k-1} /(k-1)!$\\
%on a même $exp(adx)\in aut (L)$, et plus généralement, si on remplace $adx$ par une dérivation nilpotente $\delta$ de L , ce qui précede reste vrai,\\
%pour montrer ça on utilise la régle de Leibniz:\\
%$\frac{\delta^{n}}{n !}(x y)=\sum_{i=0}^{n}(1 / i !)\left(\delta^{i} x\right)(1 /(n-i) !)\left(\delta^{n-i} y\right)$ , \\
%pour la suite de calcul on utilise le fair que  $(\delta^k=0)$
%$\begin{aligned} \exp \delta(x) \exp \delta(y) &=\left(\sum_{i=0}^{k-1}\left(\frac{\delta^{i} x}{i !}\right)\right)\left(\sum_{j=0}^{k-1}\left(\frac{\delta^{j} y}{j !}\right)\right) \\ &=\sum_{n=0}^{2 k-2}\left(\sum_{i=0}^{n}\left(\frac{\delta^{i} x}{i !}\right)\left(\frac{\delta^{n-i} y}{(n-i) !}\right)\right) \\ &=\sum_{n=0}^{2 k-2} \frac{\delta^{n}(x y)}{n !} \\ &=\sum_{n=0}^{k-1} \frac{\delta^{n}(x y)}{n !} \\ &=\exp \delta(x y) \end{aligned}$
%\begin{df}
%Un automorphisme de la forme $esp(adx),$ avec adx nilpotent est appelé un interne, plus généralement , le %sous groupe de $Aut L$ géneré par ces éléments est noté $Int L$ et appelée l'automorphisme intérieur.
%\end{df}
%\begin{ex}
%$L=sl(2,F)$ avec la base standard $(x,y,h)$\\
%on définit: $\sigma=\exp \text { ad } x \cdot \exp \text { ad }(-y) \cdot \exp \text { ad } x $, $(\sigma %%%\in \text { Int } L)$\\
%on a $\sigma(x)=-y, \sigma(y)=-x, \sigma(h)=-h$\\
%et en particulier $\sigma$ est d'ordre 2, noter que $exp(x)$ et $exp(-y)$ sont des éléments bien définies %
%de $sl(2,F)$ la conjugaison pour laquelle L est invariant.\\
%donc le produit $s=(expx)(exp(-y)$ induit un automorphisme $z:\longmapsto szs^{-1}$ de L.Un simple calcul %montre que $s=\left(\begin{array}{rr}0 & 1 \\ -1 & 0\end{array}\right)$ et que la conjugaison par s aa le %même effet sur L que l'application de $\sigma$
%\end{ex}

\section{Algèbre de Lie solvable et nilpotente}
...
\begin{df}
\quad Une algèbre de Lie $L$ est dite \textbf{solvable} si : $\exists n \in \NN $ tel que $L^{(n)}=0$.
\end{df}
...
\begin{itemize}
\item[(a)] Si $L$ est solvable, alors toutes ses sous-algèbres et les images de ses homomorphismes sont solvables.
\item[(b)] Si $I$ est un idéal solvable de $L$ tel que $L/I$ est solvable alors $L$ aussi est solvable.
\item[(c)] Si $I$ , $J$ deux idéaux solvables de $L$, alors $I+J$ est solvable.
\end{itemize}
\end{prop}
...
\item[(c)] Le (d) du théorème (\ref{thm:iso}) donne un isomorphisme entre $(I+J) / J$ et $I /(I \cap J)$ comme image d'homomorphisme de $I$, $(I+J) / J$  est solvable donc d'après (b)  $(I+J)$ est solvable.
\end{enumerate}
\end{prv}

\begin{df}
\quad On dit que $I$ est un idéal \textbf{solvable maximal} si il n'est inclut dans aucun idéal solvable. Cet idéal est unique et appelée \textbf{radical} de L, noté $Rad(L)$
\end{df}
...
En effet: $L$ est simple donc il n'a que deux idéaux ${0}$ et $L$. Or $L$ n'est pas solvable donc $Rad L= {0}$.
...
\begin{thm}[Critère de Cartan]\label{th:cartan}
